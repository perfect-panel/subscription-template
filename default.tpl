...
vless://{{ $password }}@{{ $server }}:{{ $proxy.Port }}?encryption=none{{- if ne (default "" $proxy.Flow) "" }}&flow={{ $proxy.Flow }}{{- end }}{{- if ne $proxy.Transport "" }}&type={{ (ternary "ws" $proxy.Transport (eq $proxy.Transport "websocket")) }}{{- end }}{{- if and (or (eq $proxy.Transport "ws") (eq $proxy.Transport "websocket") (eq $proxy.Transport "xhttp") (eq $proxy.Transport "httpupgrade")) (ne (default "" $proxy.Host) "") }}&host={{ $proxy.Host }}{{- end }}{{- if and (or (eq $proxy.Transport "ws") (eq $proxy.Transport "websocket") (eq $proxy.Transport "xhttp") (eq $proxy.Transport "httpupgrade")) (ne (default "" $proxy.Path) "") }}&path={{ $proxy.Path | urlquery }}{{- end }}{{- if and (eq $proxy.Transport "grpc") (ne (default "" $proxy.ServiceName) "") }}&serviceName={{ $proxy.ServiceName }}{{- end }}{{- if or (eq $proxy.Security "tls") (eq $proxy.Security "reality") }}&security={{ $proxy.Security }}{{- end }}{{- if ne $sni "" }}&sni={{ $sni }}{{- end }}{{- if $proxy.AllowInsecure }}&allowInsecure=1{{- end }}{{- if ne (default "" $proxy.Fingerprint) "" }}&fp={{ $proxy.Fingerprint }}{{- end }}{{- if and (eq $proxy.Security "reality") (ne (default "" $proxy.RealityPublicKey) "") }}&pbk={{ $proxy.RealityPublicKey }}{{- end }}{{- if and (eq $proxy.Security "reality") (ne (default "" $proxy.RealityShortId) "") }}&sid={{ $proxy.RealityShortId }}{{- end }}&{{ $common }}#{{ $proxy.Name }}
